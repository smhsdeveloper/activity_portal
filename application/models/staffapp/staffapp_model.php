...
class staffapp_model extends CI_Model {
    ...
    public function punchActivity($postDataObj) {

        $workdate = date('Y-m-d', strtotime($postDataObj['activity_date']));
        $starttime = strtotime($postDataObj['start_timer']);
        $stoptime = strtotime($postDataObj['stop_timer']);
        if ($stoptime < $starttime) {
            return 'invalidtime';
        }
        $timediffrnce = $stoptime - $starttime;
        $totaltime = gmdate('H:i:s', $timediffrnce);
        ...
        $dataArray = array(
            ...
            'activity_date' => $workdate,
            'hour_worked' => $totaltime,
        ...
        $this->savePunchDetail($lastInsertId, $postDataObj);
        if ($lastInsertId > 0) {
    ...
    public function savePunchDetail($id, $postDataObj) {

        $starttime = date('H:i:s', strtotime($postDataObj['start_timer']));
        $stoptime = date('H:i:s', strtotime($postDataObj['stop_timer']));
    ...
    public function checkLoginDetail($postDataObj) {
        $staffData = array();
        $this->db->select('a.id,a.name,a.designation,a.email_address,a.mobile_for_sms');
        $this->db->join('system_users as b', 'b.staff_id=a.id');
        $this->db->where('b.pass', $postDataObj['password']);
        $staffData = $this->db->get_where('company_staff_details as a', array('a.email_address' => $postDataObj['staff_email']))->row_array();
        ...
        $hrwrked['hour_worked'] = $this->db->get_where('acr_emp_activity_detail', array('staff_id' => $staffData['id'], 'activity_date' => date('Y-m-d')))->row_array();
        if (empty($hrwrked['hour_worked'])) {
            $staffData['loginstatus'] = 'first';
        } else {
            $staffData['loginstatus'] = 'notfirst';
        }
...
//            $totalWeekWrkTime = gmdate('H:i', $weekHr_Wrkd);
            if ($weekHr_Wrkd == '0') {
                $totalWeekWrkTime = 'NA';
            } else {
                $hours = floor($weekHr_Wrkd / 3600);
               if($hours == '0'){
                   $hours = '00';
               }
                $minutes = floor(($weekHr_Wrkd / 60) % 60);
                $totalWeekWrkTime = $hours.':'.$minutes . ' Hrs';
            }
            $todayWrkTime = 0;
            $this->db->select('sum(TIME_TO_SEC(hour_worked)) AS hour_worked');
            ...
            $todayTotalWrkTime = gmdate('H:i', $todayWrkTime);
            if ($todayTotalWrkTime == '00:00') {
                $todayTotalWrkTime = 'NA';
            } else {
                $todayTotalWrkTime = $todayTotalWrkTime . ' Hrs';
            }
    ...
    public function projctDshboardDataInMins($postDataObj) {
        $staffaccessKey = $this->checkStaffAccesskey($postDataObj);
        if (!empty($staffaccessKey)) {
            $todaydate = date('Y-m-d');
            $dayss = date("l", strtotime($todaydate));
            ...
            if ($totalWeekWrkTime == '0') {
                $totalWeekWrkTime = 'NA';
            } else {

               $totalWeekWrkTime = floor ($weekHr_Wrkd / 60);
                $totalWeekWrkTime = $totalWeekWrkTime . ' Mins';
            }
...
//            $todayTotalWrkTime = gmdate('H:i', $todayWrkTime);
            if ($todayWrkTime == '0') {
                ...
                $todayTotalWrkTime = floor ($todayWrkTime / 60);
                $todayTotalWrkTime = $todayTotalWrkTime . ' Mins';
            }
            return array('totalWeekWrkedHr' => $totalWeekWrkTime, 'todayWrkTime' => $todayTotalWrkTime);
        } else {
            return -1;
   ...
   public function getholiday($date) {
    ...
    public function getTaskDetails($postDataObj) {
        ...
        if (!empty($staffaccessKey)) {
            ...
            foreach ($data as $value) {
                ...
                $value['pause_time'] = date('h:i A', strtotime($value['pause_time']));
                if( $value['start_time'] ===  $value['pause_time']){
                     unset($value);
            ...
            $taskdata['data']['hour_worked'] = date('i', strtotime($taskdata['data'][0]['hour_worked']));
            $mydata=  array_values(array_filter($mydata));
            $taskdata['time'] = $mydata;
            return $taskdata;
        } else {
            return -1;
        }
    ...
    public function projectFullData($postDataObj) {
        $taskdata = array();
        $taskdata =  $this->getTaskDetails($postDataObj);
        $taskdata['dashboard'] = $this->projctDshboardDataInMins($postDataObj);
        $taskdata['getFullTaskDetail'] = $this->getFullTaskDetail($postDataObj);
        return $taskdata;
    ...
    public function stopTask($postDataObj) {
        ...
        if (!empty($staffaccessKey)) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                $data['todaytotaltime'] = $todayTotalWrkTime;
                return $data;
            }
        } else {
            return -1;
